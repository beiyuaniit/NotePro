...
- 五大数据类型
  ...
  - String
    ...
    - 底层是一个简单的动态字符串，可以存储字符串，数字和二进制数据
    - 二进制安全的（只存原始二进制，不对特殊字符转义）。意味着Redis的string可以包含任何数据。比如jpg图片或者序列化的对象
...
- AOF（Append Only File）
  ...
  - 命令重写机制：AOF采用文件追加方式，文件会越来越大为避免出现此种情况，新增了重写机制, 当AOF文件的大小超过所设定的阈值时，Redis就会fork子进程，进行AOF文件的内容压缩， 只保留可以恢复数据的最小指令集.可以使用命令bgrewriteaof。如set a a和set b b可合并为set a a b b
...
- 缓存穿透
  - 客户端访问不存在的数据，使得请求直达存储层，导致负载过大，直至宕机。
...
- Redis采用的是单线程+IO多路复用技术。
  - 这里单线程指的是Redis中读写操作和网络IO是由一个线程来完成的，但是其他操作时由其他线程完成的，例如：持久化操作、异步删除、集群数据同步等。
...
- 在大型网络应用中，单个redis很难保证数据量的访问于存储，所以可以搭建分布式系统，保证数据的分撒储存与数据的一致性，实现redis的高可用。 
  ...
  - 集群模式（水平方向扩展）：采用无中心节点的方式实现。底层可以多个主服务器相连，一个主服务器可以有多个从服务器，不同的主服务器存储不同的数据。优点：可用性更高，内存可用性高。（每个节点的压力更新
...
- 定期删除：redis会将设置了过期时间的key保存到一个字典里面，然后每过一定时间就扫描一次
  - LRU （最近最少使用原则）   
...
![rdb和aof](B:/我的坚果云/面试题/images/rdb和aof.png)
...
![延迟双删](B:/我的坚果云/面试题/images/延迟双删.png)
...
![延迟删除2](B:/我的坚果云/面试题/images/延迟删除2.png)
...
![先更新库后删缓存](B:/我的坚果云/面试题/images/先更新库后删缓存.png)
...
![先更新库后删缓存2](B:/我的坚果云/面试题/images/先更新库后删缓存2.png)
...
![缓存穿透](B:/我的坚果云/面试题/images/缓存穿透.png)
...
![Redis线程模型](B:/我的坚果云/面试题/images/Redis线程模型.png)
...
![Redis线程模型2](B:/我的坚果云/面试题/images/Redis线程模型2.png)
...
![Redis多线程机制](B:/我的坚果云/面试题/images/Redis多线程机制.png)
...
- **相同点：**都采用了 Master 线 程 -Worker 线程的模型。
...
Redis的事务并不是我们传统意义上理解的事务，我们都知道 ，但 Redis 没有在事务上增加任何维持原子性的机制，所以 Redis **事务的执行并不是原子性的**。
...
![Redis单副本](B:/我的坚果云/面试题/images/Redis单副本.png)
...
![Redis主从](B:/我的坚果云/面试题/images/Redis主从.png)
...
可自动切换到从服务器，实现故障发现、故障自动转移、配置中心和客户端通知等功能
...
![Redis哨兵](B:/我的坚果云/面试题/images/Redis哨兵.png)
...
![Redis集群](B:/我的坚果云/面试题/images/Redis集群.png)

**优点：**

* 无中心架构；
* 数据按照slot存储分布在多个节点，节点间数据共享，可动态调整数据分布；
* 可扩展性：可线性扩展到1000多个节点左右，节点可动态添加或删除；
...
![Redis自研](B:/我的坚果云/面试题/images/Redis自研.png)
...
开始full resynchronization的时候，master会启动一个后台线程，开始生成一份RDB快照文件，同时还会将从客户端收到的所有写命令缓存在内存中AOF。RDB文件生成完毕之后，master会将这个RDB发送给slave，slave会先写入本地磁盘，然后再从本地磁盘加载到内存中。然后master会将内存中缓存的写命令发送给slave，slave也会同步这些数据。
...
使用setnx来加锁。key是锁的唯一标识，按业务来决定命名，value这里设置为test。  防止超时可设置过期时间，但是setnx不支持传入时间参数，下面用set完成setnx并加上超时时间
...
​	通过对锁进行重入计数，加锁时加 1，解锁时减 1，当计数归 0 时释放锁。
...
![RedLock原理](B:/我的坚果云/面试题/images/RedLock原理.png)
