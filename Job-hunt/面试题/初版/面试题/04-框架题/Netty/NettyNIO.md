...
stream单向数据通道，channel双向数据通道，可用于输入和输出数据
...
channel有请求则通知selector，由selector进行调度

## ByteBuffer

本身是一个抽象类，可通过其静态方法allocate获取一个缓冲区
...
public class TestByteBuffer {
    public static void main(String[] args) {
        ...
        try (FileChannel channel = new FileInputStream("stu.txt").getChannel()) {
            ...
            //一次只能写满缓冲区的10个字节，但缓冲区不能跟着文件大小来，所以用while来多次读取
            while((hasNext = channel.read(buffer)) > 0) { //从channel写入数据到buffer
                // 切换模式 limit=position, position=0
                buffer.flip();//buffer切换到读模式
                // 当buffer中还有数据时，获取其中的数据
                while(buffer.hasRemaining()) {
                    builder.append((char)buffer.get());  //读取到的是Byte，转为char
                }
                // 清理后 切换模式 position=0, limit=capacity
                buffer.clear();
            }
...
- **limit**：缓冲区的界限。位于limit 后的数据不可读写。缓冲区的限制不能为负，并且**不能大于其容量**。是为了限制应用程序不要操作还没放有数据的区域
...
- 进行该操作后
  - 如果是写模式->读模式，position = 0 ， limit 指向最后一个元素的下一个位置，capacity不变。所以读的话是从头开始读
  - 如果是读->写，则恢复为put()方法中的值，也就是最新的位置
...
//直接内存，少了一次拷贝，效率更高，不会发生GC。要调用操作系统函数，内存分配效率低点。netty对这个进行了改进
...
发送方在发送数据时，并不是一条一条地发送数据，而是**将数据整合在一起**，当数据达到一定的数量后再一起发送。这就会导致多条信息被放在一个缓冲区中被一起发送出去。相当于快递员，等到快递够了再统一发出，这样效率才高   
        ...
        for(int i = 0; i < buffer.limit(); i++) {
            ...
            if (buffer.get(i) == '\n') {
                ...
                for(int j = 0; j < length; j++) {
                    ...
                    target.put(buffer.get());  //但是这里get()读取数据会改变position的位置
                }
                // 打印查看结果
...
public class TestChannel {
    public static void main(String[] args){
    //会覆盖to中文件内容
        try (FileChannel from = new FileInputStream("stu.txt").getChannel();
             FileChannel to = new FileOutputStream("student.txt").getChannel();) {
            // 参数：inputChannel的起始位置，传输数据的大小，目的channel
            // 返回值为传输的数据的字节数
            // transferTo一次只能传输2G的数据
            
            //效率高，底层使用操作系统的零拷贝技术优化
...
Path source = Paths.get("d:\\1.txt"); // 反斜杠要转义，绝对路径 代表了  d:\1.txt 反斜杠需要转义

Path source = Paths.get("d:/1.txt"); // 正斜杠不用，绝对路径 同样代表了  d:\1.txt
...
- 缺点
  - **不适合并发量大的网络开发**
  - **每个请求都会分配一个线程操作（因为你这个线程干不了其他事），系统开销大**
    ...
    public static void main(String[] args) {
        ...
        try(ServerSocketChannel server = ServerSocketChannel.open()) {
            ...
            while (true) {
                ...
                System.out.println("after connecting...");    //索引这行代码在没有连接的时候不会
...
public class Client {
    public static void main(String[] args) {
        try (SocketChannel socketChannel = SocketChannel.open()) {
            ...
            socketChannel.write(Charset.defaultCharset().encode("hello"));//发送数据
            ...
            while (true) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if(key.isAcceptable()) {
                        ...
                        System.out.println("before reading...");
                        int state=channel.read(buffer);
                        System.out.println(StandardCharsets.UTF_8.decode(buffer));
                        if(state==-1){
                        	//客户端断开会产生一个读事件，当返回-1时应该把它移除
                        	key.cancel();
                        }
                        System.out.println("after reading...");
                        buffer.flip();
            ...
            while (true) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if(key.isAcceptable()) {
                        ...
                        SocketChannel socketChannel = channel.accept();
                        System.out.println("after accepting...");
                        // 设置为非阻塞模式，同时将连接的通道也注册到选择其中，同时设置附件，使每个channel关联自己的buffer
                        socketChannel.configureBlocking(false);
                        ByteBuffer buffer = ByteBuffer.allocate(16);
                        ...
                        if(read == -1) {
                            ...
                            split(buffer);
                            // 如果缓冲区太小，就进行扩容
                            if (buffer.position() == buffer.limit()) {//没有读到换行符，buffer.compact()压缩后还是原来长度，进行扩容，直到读取到分隔符
                                ByteBuffer newBuffer = ByteBuffer.allocate(buffer.capacity()*2);
                                // 将旧buffer中的内容放入新的buffer中
                                buffer.flip();
