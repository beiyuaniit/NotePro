...
​	使用继承方式的好处是，在run（）方法内获取当前线程直接使用this就可以了，无须使用Thread.currentThread（）方法；不好的地	方是Java不支持多继承，如果继承了Thread类，那么就不能再继承其他类。另外任务与代码没有分离，当多个线程执行一样的任务时需	要多份任务代码
...
- 当竞争非常激烈，多个线程同时竞争轻量级锁（一般来说就是当前线程数为CPU核数的一半），就会将轻量级锁膨胀为重量级锁
  ...
  或者当程序中调用了Object的wait方法，直接会将锁膨胀为重量级锁，无论当前竞争激烈与否，因为wait方法实际上需要对象monitor实现
...
程仅能判断出共享变量的值与初值 A **是否相同**，不能感知到这种从 A 改为 B 又 改回 A 的情况，如果主线程希望：
只要有其它线程【**动过了**】共享变量，那么自己的 **cas 就算失败**，这时，仅比较值是不够的，需要再加一个**版本号**

### **AtomicStampedReference**

增加一个版本号判断，值和版本号都相同，则设置值并更新stamp
...
不太对，好像可以通过public static Unsafe getUnsafe()
...
有多个任务则启动多个线程运行任务，最多coreSize个工作线程

当少于coreSize个任务或没有任务时，则工作线程也相应减少
...
- keepAliveTime：救急线程空闲时的最大生存时间 (空闲线程)
- unit：时间单位
- workQueue：阻塞队列（存放任务）
...
- 也可以自己实现`RejectedExecutionHandler`接口：
...
```java
    ...
    public void test01() throws InterruptedException {
        ...
        ThreadPoolExecutor executor=new ThreadPoolExecutor(5,7,10, TimeUnit.SECONDS,blockingQueue,threadFactory);
