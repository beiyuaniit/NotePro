...
- 堆内存诊断
  ...
  **jps**
  ...
  ​	Java Process Status Tool 的简称,它的作用是为了列出所有正在运行中的 Java 虚拟机进程
...
- 存放什么
  ...
  ![img](https://img-blog.csdnimg.cn/20200501184250971.png)
...
- 复制
  ...
  - ​	换FROM和TO。这样也可以避免内存碎片的问题，但是会占用双倍的内存空间。
- 设置两个幸存者区最大的好处是**解决内存碎片化**。也方便整理
...
在制定文件储存格式规范时，规范制定者可以选择任何魔数，只要是还没有被广泛使用的。

例如，PDF文件魔数使用的文本值"%PDF"（ASCII字符），底层编码十六进制形式（25 50 44 46）；GIF文件的魔数使用的文本值"GIF89a"（ASCII字符），底层编码十六进制形式 (47 49 46 38 39 61) 。Java的Class文件魔数使用数值“CAFEBABE”（十六进制），底层编码就是该数值。
...
在JDK 5以后，它们的转换可以在编译期自动完成，所以运行时性能也不会下降
